@extends('site.layout', ['title' => $post->seo_title . ' - 帝国 CMS'])
        ...
        <section class="overflow-hidden rounded-[32px] border border-stone-200/80 bg-white/90 shadow-sm">
            @if ($post->isFlashModel())
                <div class="p-6 sm:p-8 lg:p-10">
                    ...
                    @if (filled($post->content))
                        <div class="prose prose-stone mt-10 max-w-none leading-8 prose-headings:font-semibold prose-a:text-amber-700 prose-img:rounded-[24px]">
            ...
            @else
                ...
                <div class="p-6 sm:p-8 lg:p-10">
                    ...
                    <div class="prose prose-stone mt-10 max-w-none leading-8 prose-headings:font-semibold prose-a:text-amber-700 prose-img:rounded-[24px]">
                        {!! filled($post->content) ? $post->renderRichContent('content') : '<p>当前文章还没有正文内容。</p>' !!}
                    </div>
                </div>
            @endif
        </section>
            ...
            <div class="rounded-[28px] border border-stone-200/80 bg-white/90 p-6 shadow-sm">
                ...
                <div class="mt-6 grid gap-3 sm:grid-cols-2">
                    @forelse ($attachments as $attachment)
                        <a href="{{ $attachment->getUrl() }}" target="_blank" rel="noreferrer" class="group rounded-[24px] border border-stone-200 bg-[linear-gradient(180deg,#fafaf9_0%,#f5f5f4_100%)] px-5 py-4 transition hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-sm">
        ...
        <section class="rounded-[30px] border border-stone-200/80 bg-white/90 p-6 shadow-sm sm:p-8">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div class="rounded-full bg-stone-100 px-4 py-2 text-sm text-stone-600">
                    已公开 {{ $commentThreads->count() }} 个讨论主题
                </div>
            </div>
                ...
                @auth
                    <form method="POST" action="{{ route('posts.comments.store', $post->slug) }}" class="space-y-4">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <div class="rounded-full bg-white px-3 py-1 text-xs text-stone-500">当前登录：{{ auth()->user()->username }}</div>
                        </div>
                        <textarea
                            id="body"
                            name="body"
            ...
            <div class="mt-8 space-y-4">
                @forelse ($commentThreads as $comment)
                    <article class="rounded-[26px] border border-stone-200 bg-stone-50/90 p-5 shadow-sm">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-stone-800 to-stone-600 text-sm font-semibold text-white">
                                    {{ mb_substr($comment->user?->username ?? '匿', 0, 1) }}
                                </div>
                                <div>
                                    <div class="font-medium text-stone-900">{{ $comment->user?->username ?? '匿名用户' }}</div>
                                    <div class="text-xs text-stone-500">{{ optional($comment->created_at)->format('Y-m-d H:i') }}</div>
                                </div>
                            </div>
                            <div class="rounded-full bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">Thread</div>
                        </div>
                        ...
                        @auth
                            <form method="POST" action="{{ route('posts.comments.store', $post->slug) }}" class="mt-4 rounded-[22px] border border-stone-200/80 bg-white px-4 py-4">
                                @csrf
                                <input type="hidden" name="parent_id" value="{{ $comment->id }}">
                                <div class="flex items-center justify-between gap-3">
                                    <label for="reply-{{ $comment->id }}" class="text-sm font-medium text-stone-800">回复这条评论</label>
                                    <div class="rounded-full bg-amber-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-amber-700">Reply</div>
                                </div>
                                <textarea
                                    id="reply-{{ $comment->id }}"
                                    name="body"
                                    ...
                                    placeholder="补充观点、追问细节，或直接回复楼主。"
                                >{{ old('parent_id') == $comment->id ? old('body') : '' }}</textarea>
                                <div class="mt-3 flex items-center justify-between gap-3">
                                    <p class="text-xs leading-6 text-stone-500">回复同样会先进入审核，审核通过后再显示在当前讨论串里。</p>
                        ...
                        @if ($comment->children->isNotEmpty())
                            <div class="mt-4 space-y-3 border-l-2 border-amber-200 pl-4 sm:pl-5">
                                @foreach ($comment->children as $reply)
                                    <article class="rounded-[22px] border border-stone-200/80 bg-white px-4 py-4">
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="flex items-center gap-3">
                                                <div class="flex h-9 w-9 items-center justify-center rounded-2xl bg-amber-100 text-xs font-semibold text-amber-800">
                                                    {{ mb_substr($reply->user?->username ?? '匿', 0, 1) }}
                                                </div>
                                                <div>
                                                    <div class="text-sm font-medium text-stone-900">{{ $reply->user?->username ?? '匿名用户' }}</div>
                                                    <div class="text-xs text-stone-500">{{ optional($reply->created_at)->format('Y-m-d H:i') }}</div>
                                                </div>
                                            </div>
                                            <div class="rounded-full bg-amber-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-amber-700">Reply</div>
                                        </div>
                                        <div class="mt-3 text-sm leading-7 text-stone-700">
                                            @if ($reply->parent?->user)
                                                <span class="mr-2 inline-flex rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-medium text-amber-700">回复 {{ $reply->parent->user->username }}</span>
                                            @endif
                                            {{ $reply->content }}
                                        </div>
                                    </article>
