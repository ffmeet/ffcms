@php
    ...
    $isWebp = ! $isFolder && strtolower((string) $item->extension) === 'webp';
    $isRecentUpload = ! $isFolder && $item->created_at?->gt(now()->subMinutes(10));
@endphp
...
>
    <div class="fi-media-item-thumbnail-container">
        ...
        @else
            ...
            <div class="fi-media-item-badge-stack">
                @if ($isRecentUpload)
                    <span class="fi-media-item-flag fi-media-item-flag-new">NEW</span>
                @endif

                @if ($isWebp)
                    <span class="fi-media-item-flag fi-media-item-flag-webp">WEBP</span>
                @endif
            </div>
        @endif
        ...
        <div class="absolute inset-0 z-10 cursor-pointer" x-on:click.stop="handleSingleClick"></div>
    </div>

    <div class="fi-media-item-info">
        <div class="min-w-0">
            <h4
                class="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate flex items-center gap-1.5"
                title="{{ $item->name }}"
            >
                @if ($isFolder)
    ...
    <div class="absolute inset-x-0 bottom-0 p-2 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-200 z-30 pointer-events-none">
        <div class="flex justify-center gap-1 pointer-events-auto"></div>
    </div>
</div>
