@extends(\App\Support\SiteTheme::view('layout', 'themes.default.layout'), ['title' => '会员中心'])
        ...
        <div class="space-y-6">
                ...
                <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    ...
                    <div class="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-7">
                        <div class="rounded-[12px] border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                            ...
                            <div class="mt-1 text-2xl font-semibold text-slate-900">{{ $metrics['drafts'] ?? 0 }}</div>
                        </div>
                        <div class="rounded-[12px] border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                            <div class="text-sm text-slate-500">待审</div>
                            <div class="mt-1 text-2xl font-semibold text-slate-900">{{ $metrics['pending_posts'] ?? 0 }}</div>
                        </div>
                        <div class="rounded-[12px] border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                            <div class="text-sm text-slate-500">已发布</div>
                            <div class="mt-1 text-2xl font-semibold text-slate-900">{{ $metrics['published_posts'] ?? 0 }}</div>
                        </div>
                        <div class="rounded-[12px] border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                            <div class="text-sm text-slate-500">评论</div>
                            <div class="mt-1 text-2xl font-semibold text-slate-900">{{ $metrics['comments'] ?? 0 }}</div>
                        </div>
                        <div class="rounded-[12px] border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                            <div class="text-sm text-slate-500">订单</div>
                            <div class="mt-1 text-2xl font-semibold text-slate-900">{{ $metrics['orders'] ?? 0 }}</div>
                        </div>
                        <div class="rounded-[12px] border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                            <div class="text-sm text-slate-500">订阅</div>
                            <div class="mt-1 text-2xl font-semibold text-slate-900">{{ $metrics['subscriptions'] ?? 0 }}</div>
                        </div>
                        <div class="rounded-[12px] border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                            <div class="text-sm text-slate-500">报名</div>
                            <div class="mt-1 text-2xl font-semibold text-slate-900">{{ $metrics['registrations'] ?? 0 }}</div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="grid gap-4 xl:grid-cols-2">
                @foreach (($attentionCards ?? []) as $card)
                    <article class="rounded-[16px] border border-slate-200 bg-white p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
