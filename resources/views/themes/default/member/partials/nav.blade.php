    ...
    <section class="rounded-[18px] border border-slate-200/80 bg-white/96 p-5 shadow-[0_20px_50px_rgba(15,23,42,0.08)]">
        <div class="flex flex-col items-center text-center">
            @if ($memberAvatarMedium)
                <img src="{{ $memberAvatarMedium }}" alt="{{ $memberUser?->public_display_name }}" class="h-24 w-24 rounded-full object-cover shadow-[0_18px_44px_rgba(37,99,235,0.20)]">
            @else
            ...
            @endif
            <h2 class="mt-4 text-2xl font-semibold tracking-tight text-slate-900">{{ $memberUser?->public_display_name }}</h2>
    ...
    <section class="rounded-[18px] border border-slate-200/80 bg-white/96 p-4 shadow-[0_20px_50px_rgba(15,23,42,0.08)]">
        <div class="flex items-center justify-between">
            ...
            <span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
                你好，{{ $memberUser?->public_display_name }}
            </span>
        </div>
